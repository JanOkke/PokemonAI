...
def get_exp(growth_rate: str, level: int) -> int:
    ...
    if growth_rate == growth.FLUCTUATING:
        if level <= settings.MAX_LEVEL:
            if level <= 15:
                return round(level ** 3 * ((24 + round((level + 1) / 3)) / 50))
            if level <= 36:
                return round(level ** 3 * ((14 + level)/50))
            if level > 36:
                return round(level ** 3 * ((32 + round(level / 2)) / 50))
        else:
            raise WrongLevel(level)
...
def min_max_exp(growth_rate: str, level: int) -> [int, int]:
    min_exp = get_exp(growth_rate, level)
    max_exp = get_exp(growth_rate, level + 1) - 1
